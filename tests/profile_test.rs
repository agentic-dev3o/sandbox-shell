use sx::config::profile::{Profile, load_profile, load_profiles, compose_profiles, BuiltinProfile};
...
fn test_builtin_profile_gpg() {
    ...
    assert!(profile.filesystem.allow_read.iter().any(|p| p.contains(".gnupg")));
}
...
"#).unwrap();
    ...
    assert!(profile.filesystem.allow_read.contains(&"~/custom-path".to_string()));
}
...
"#).unwrap();
    ...
    assert!(profiles[0].filesystem.allow_read.contains(&"~/mypath".to_string()));
}
...
fn test_compose_profiles_single() {
    ...
    assert_eq!(composed.filesystem.allow_read, profile.filesystem.allow_read);
}
