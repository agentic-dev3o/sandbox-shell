...
pub fn explain(args: &Args) -> Result<()> {
    ...
    if let Some(cmd) = &args.command {
        ...
        let shell = context.config.sandbox.shell.clone()
...
fn load_effective_config(args: &Args, working_dir: &PathBuf) -> Result<Config> {
    if args.no_config {
        return Ok(Config::default());
    }

    // Load global config
    let global = load_global_config(args.config.as_deref())
        .context("Failed to load global config")?;

    // Load project config
    let project = load_project_config(working_dir)
...
fn build_sandbox_params(
    ...
    // Expand all paths
    allow_read = expand_paths(&allow_read).into_iter().map(|p| p.to_string_lossy().to_string()).collect();
    deny_read = expand_paths(&deny_read).into_iter().map(|p| p.to_string_lossy().to_string()).collect();
    allow_write = expand_paths(&allow_write).into_iter().map(|p| p.to_string_lossy().to_string()).collect();
...
fn determine_network_mode(args: &Args, profile: &Profile, config: &Config) -> NetworkMode {
    ...
    config.sandbox.network.unwrap_or(config.sandbox.default_network)
}
